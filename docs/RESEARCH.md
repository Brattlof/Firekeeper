...
| FK-14 | Can a camp object on the fire be recognised from its nameplate, so `/fk place` becomes a fallback rather than the only way? | Whether the camp fills itself in, `Core/Camp.lua` | S'more Skills uses `C_NamePlate.GetNamePlates` to spot fires. Stand at a fire with objects on it and report what `/dump #C_NamePlate.GetNamePlates()` gives, and whether any plate carries an object's name |
