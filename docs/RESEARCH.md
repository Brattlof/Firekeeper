...
| FK-15 | Is the layer of a layered realm readable, and is `UnitGUID` secret in ordinary play? | Whether `/fk find` can say a camp is unreachable, `Core/Discovery.lua` | `UnitGUID` is `SecretWhenUnitIdentityRestricted`, so it is tested with `issecretvalue` before being split. `diagnostics.layer` records the number or `not readable`: stand near any creature, type `/fk find`, then `/reload` |

Reading other people's Forever addons answers these faster than another `/reload` does.
Four of the answers above came from source and README notes rather than from the client:
the addon-message result code (FK-4), the combat aura behaviour (FK-10), the absence of a
campsite API (FK-5), and the nameplate idea (FK-14). Check there first.
